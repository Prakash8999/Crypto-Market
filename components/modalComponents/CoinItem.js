...
const CoinItem = ({
  ...
  useEffect(() => {
    ...
    const getImgUrl = async () => {
      const imgUrl = builder.image(token&&token.logo.asset._ref).url()
      setImageUrl(imgUrl)
    }
  ...
  return (
    <Wrapper
      style={{
        backgroundColor: selectedToken && selectedToken.name === token && token.name && '#141519',
      }}
      onClick={() => {
    ...
    >
      ...
      <IsSelected>
        {Boolean(selectedToken && selectedToken.contractAddress === token&&token.contractAddress) && (
          <FaCheck />
        )}
      </IsSelected>
    </Wrapper>
