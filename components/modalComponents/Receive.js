...
const Receive = ({ setAction, selectedToken, walletAddress }) => {
  ...
  useEffect(() => {
    const url = builder.image(selectedToken && selectedToken.logo.asset._ref).url()
    setImageUrl(url)
  ...
  return (
    <Wrapper>
      <Content>
        ...
        <Row>
          <div>
            <Title>{selectedToken && selectedToken.symbol} Address</Title>
            <Address>{selectedToken && selectedToken.contractAddress}</Address>
          </div>
          <CopyButton
            onClick={() => {
